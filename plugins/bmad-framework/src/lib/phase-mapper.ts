...
export class BMADPhaseMapper {
  ...
  getNextPhase(currentPhase: BMADPhase): BMADPhase | null {
    const phases: BMADPhase[] = ['brainstorming', 'architecture', 'development', 'testing'];
   ...
   */
  private extractFeatureStatus(feature: Feature): string {
   ...
   */
  private statusToPhase(status: string): BMADPhase {
   ...
   */
  private assessComplexity(feature: Feature): BMADComplexity {
    ...
    if (
      description.includes('very complex') ||
   ...
   */
  private estimateDuration(phase: BMADPhase, complexity: BMADComplexity): number {
   ...
   */
  private calculateMappingConfidence(feature: Feature, phase: BMADPhase): number {
   ...
   */
  private getPersonaForPhase(phase: BMADPhase): BMADAgentRole {
    const phaseInfo = BMAD_PHASES[phase];
    // Return the first required role (primary)
    return phaseInfo.requiredRoles[0] as BMADAgentRole;
  }

  /**
   * Calculate phase progress for a feature
   */
  private calculatePhaseProgress(feature: Feature, currentPhase: BMADPhase): BMADPhaseProgress {
    const phases: BMADPhase[] = ['brainstorming', 'architecture', 'development', 'testing'];
    const currentIndex = phases.indexOf(currentPhase);
   ...
   */
  private estimateCompletion(feature: Feature, mapping: PhaseMappingResult): string {
    const now = new Date();
 ...
 */
export function createBMADPhaseMapper(): BMADPhaseMapper {
